...
// managePermission is the identity.Permission required to set a
// tenant's BackupPolicy, record a BackupRecord, register a Target, or
// execute a RestoreDrill.
const managePermission = identity.PermManageBackupDR

// viewPermission is the identity.Permission required for read-only
// access to a tenant's backup policies, backup record history,
// restore-drill history, and RPO/RTO evaluation results.
const viewPermission = identity.PermViewBackupDR
...
// can manage backup/DR state can also read it.
...
// falling back to uuid.Nil (which actorFor in audit.go renders as
