...
// Repository persists a Result per case, so a caller (e.g. Phase 055's
// synthesis agent) can retrieve a case's law-application analysis
// without recomputing it. Implementations:
//
//   - InMemoryRepository (this file): a fully in-memory implementation
//     backed by a map, mirroring evidenceweighing.InMemoryRepository,
//     graph.InMemoryGraphStore, and citation.InMemoryRepository's shared
//     convention of being the default, always-available implementation
//     used by tests and by any deployment that does not yet need a
//     durable backend.
type Repository interface {
	...
	// (idempotent upsert), mirroring evidenceweighing.Repository.Save's
	// overwrite convention.
	Save(ctx context.Context, result Result) error

	// Get returns the Result stored for caseID, or ErrResultNotFound if
	// none was ever saved.
	Get(ctx context.Context, caseID string) (Result, error)

	// DeleteByCase removes the Result saved for caseID. Not an error to
	// delete a case with none saved.
