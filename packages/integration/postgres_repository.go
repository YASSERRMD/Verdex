...
const credentialsColumns = `id, tenant_id, kind, secret_ref, client_id, token_url, scopes, last_verified_at, created_by, created_at, updated_at` // #nosec G101 -- this is a SQL column list, not a credential value; secret_ref is a handle, never the secret itself //nolint:gosec
