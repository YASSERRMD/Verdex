...
// update a ConnectorConfig, set ConnectorCredentials, trigger an
// inbound case import or outbound report delivery, or run
// reconciliation.
const managePermission = identity.PermManageIntegration

// viewPermission is the identity.Permission required for read-only
// access to connector configurations (minus credential material),
// import/delivery run history, and reconciliation results.
const viewPermission = identity.PermViewIntegration

// authorizeActor resolves the authenticated identity.User from ctx,
// returning ErrUnauthenticated if none is present. Mirrors
// packages/compliance.authorizeActor and
// packages/privacy.authorizeActor exactly.
...
// can manage integration configuration can also read it.
