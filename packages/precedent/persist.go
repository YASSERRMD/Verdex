...
// PersistPrecedents persists every rule's irac.RuleNode in rules via
...
// jurisdictionCode selects which corpus this call is ingesting on behalf
// of, mirroring packages/statute's PersistRules convention of persisting
// every node handed to it rather than silently filtering).
//
...
// are not rolled back, mirroring packages/statute/persist.go's
// PersistRules.
func PersistPrecedents(ctx context.Context, store graph.GraphStore, jurisdictionCode string, rules []ScoredPrecedent) ([]irac.RuleNode, error) {
	...
	for _, r := range rules {
		if err := store.CreateNode(ctx, r.RuleNode.Node); err != nil {
			return persisted, wrapPersistError(err)
		}
		persisted = append(persisted, r.RuleNode)
	}
	return persisted, nil
}

// LoadPrecedentsForJurisdiction fetches every previously persisted rule
// node listed in ruleIDs from store, returning ErrRuleNotFound (wrapping
// the underlying store error) for the first ID that cannot be found.
//
// GraphStore has no jurisdiction-scoped query, so callers are expected to
// track which rule IDs belong to a jurisdiction themselves (e.g. from the
// ids returned by PersistPrecedents) and pass them in explicitly.
...
// packages/statute/persist.go's wrapPersistError.
