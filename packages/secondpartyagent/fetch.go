...
// packages/firstpartyagent/fetch.go's own constant and rationale: a
// case's reasoning tree at Verdex's target scale is expected to
// comfortably fit within this page.
...
// from the case's tree available as evidence favorable to the second
// party for constructing an argument on it.
...
//     packages/firstpartyagent's own resolution of this edge direction),
//     and
//   - FactNodes reachable from those rules through the tree's
//     Application layer (Application --applies_to--> Rule,
//     Fact --supports--> Application).
//
// This package deliberately resolves evidence using the exact same
// structural traversal as packages/firstpartyagent — the second party is
// entitled to argue from the same case tree, favoring whichever facts and
// rules best support its own claims, rather than a restricted subset.
// "Favoring the second party" happens in the prompt (which instructs the
// model to construct the strongest good-faith case for the second party
// specifically) and in argument construction/scoring, not by hiding
...
// the only access path, matching packages/firstpartyagent's own
// convention.
func fetchIssueEvidence(ctx context.Context, api *knowledgeapi.KnowledgeAPI, caseID string, issues []issueagent.FramedIssue) ([]issueEvidence, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("secondpartyagent: fetch case tree: %w", err)
	}
