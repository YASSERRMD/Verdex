...
// managePermission is the identity.Permission required to provision or
// transition a PilotDeployment, assign/observe a PilotCase, triage a
// PilotFinding, or record/verify a RefinementRecord.
const managePermission = identity.PermManagePilot

// viewPermission is the identity.Permission required for read-only
// access to pilot deployments, cases, feedback, findings, refinements,
// and the pilot report.
const viewPermission = identity.PermViewPilot
...
// can manage the pilot can also read it.
...
// packages/vulnmanagement's equivalent guard exactly -- an actor's
