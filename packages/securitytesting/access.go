...
// managePermission is the identity.Permission required to run a
// Scenario/suite through Engine.RunSuite, transition a Finding's
// Status, or verify a remediation.
const managePermission = identity.PermManageSecuritytesting

// viewPermission is the identity.Permission required for read-only
// access to Findings, RunRecords, and the active ScopeDocument.
const viewPermission = identity.PermViewSecuritytesting
...
// can manage the security-testing harness can also read its results.
...
// packages/threatmodel's equivalent exactly -- an actor's role-level
