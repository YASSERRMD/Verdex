...
// packages/annotations's helpers_test.go newTestUser convention.
...
// mirroring packages/annotations's helpers_test.go seedCase convention.
...
// newTestService builds a caseversioning.Service backed by fresh
// InMemory repositories for both snapshots and cases, returning the
// Service along with the seeded case and tenant ID so tests can
// exercise a full snapshot/history/diff/restore round-trip without
// repeating this wiring in every test.
func newTestService(t *testing.T) (*caseversioning.Service, *caselifecycle.Case, caselifecycle.Repository, uuid.UUID) {
	t.Helper()
	...
	return svc, c, caseRepo, tenantID
}
